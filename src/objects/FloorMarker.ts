...
export function createFloorMarker(scene: THREE.Scene): void {
  ...
  // Outer ring
  const ring = new THREE.Mesh(new THREE.RingGeometry(0.55, 0.62, 64), markerMat)
  ...
  for (const xOff of [-0.18, 0.18]) {
    ...
    foot.position.set(xOff, 0.013, 0.0)
    scene.add(foot)
  }
  ...
  arrow.position.set(0, 0.014, -0.7)
  scene.add(arrow)
  ...
  labelMesh.position.set(0, 0.015, 0.9)
  scene.add(labelMesh)
}
